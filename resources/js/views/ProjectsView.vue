...
<template>
  <div class="max-w-6xl mx-auto px-4">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      <div v-for="project in projects" :key="project.id" class="bg-gray-800 rounded-xl overflow-hidden border border-gray-700 hover:border-indigo-500 transition-all duration-300 hover:shadow-xl hover:-translate-y-1 group flex flex-col">
        <div class="h-48 bg-gray-700 flex items-center justify-center overflow-hidden relative shrink-0">
           <span v-if="!project.image_path" class="text-gray-500 text-4xl font-bold opacity-20 group-hover:scale-110 transition-transform duration-500">{{ project.title.charAt(0) }}</span>
           <img v-else :src="project.image_path" :alt="project.title" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
           <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-4">
              <a v-if="project.github_url" :href="project.github_url" target="_blank" class="p-2 bg-white text-gray-900 rounded-full hover:bg-gray-200 transition" title="View Code">
              ...
              <component 
                ...
                class="p-2 bg-indigo-500 text-white rounded-full hover:bg-indigo-400 transition" 
                title="View Live"
              >
        ...
        <div class="p-6 flex flex-col flex-grow">
          ...
          <div class="flex flex-wrap gap-2 mt-auto">
            <span v-for="tag in project.tags" :key="tag" class="px-2 py-1 text-xs font-medium bg-gray-700 text-indigo-300 rounded-md border border-gray-600">
              {{ tag }}
            </span>
          </div>
        </div>
